...
- 单元/集成：20/20，通过不追踪/暂停课时、续费流水、零完成组保护、课时只扣一次、撤销/重完成、复制上次、旧版备份重复导入去重、精确 V1 数据库原地升级、畸形归档重复 ID 防护、体测历史、V1/V2 深合并与往返恢复。
- UI：6/6，通过启动、完整训练 9→8、强退后草稿/休息计时恢复、重量/次数/RPE/备注强退恢复、AX5 首页与训练控件 frame 断言；测试注入修复后已在 SE 专项复跑通过。
