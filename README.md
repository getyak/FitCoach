...
训练现场只完整展开当前未完成组，已完成和后续组收为摘要；完成后自动聚焦下一组。训练时底部只保留“完成当前组”，休息时切换为单行倒计时与“跳过”，减少滚动、视线搜索和误触。RPE 未操作时明确显示“未记录”，首次增减分别落到 7.5/6.5，不会把建议值误存为事实。教练可选开启休息结束本地提醒。另支持训练模板、体重/腰围/体脂趋势，以及包含逐组记录和课时流水的 JSON 备份。
...
- UI：7/7，通过启动、渐进式当前组自动推进、完整训练 9→8、强退后草稿/休息计时恢复、全字段恢复、数值调整后立即终止恢复、RPE 未记录语义及 AX5 当前组/休息栏 frame 断言。
