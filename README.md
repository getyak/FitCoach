...
- 单元/集成：22/22，通过不追踪/暂停课时、续费流水、零完成组保护、课时只扣一次、撤销/重完成、复制上次、旧版备份重复导入去重、精确 V1 数据库原地升级、中间版本标记后的 UUID 自愈、畸形归档重复 ID 防护、体测历史、V1/V2 深合并与往返恢复，以及系统训练深链格式校验。
- UI：10/10，通过启动、渐进式当前组自动推进、完整训练 9→8、强退后草稿/休息计时恢复、全字段恢复、数值调整后立即终止恢复、直接数值输入、精确训练深链热/冷启动、Today/训练页系统无障碍审计，以及 AX5 当前组/休息栏 frame 断言。
- 全套：32/32 通过（22 个单元/集成 + 10 个 UI），0 failed / 0 skipped。
